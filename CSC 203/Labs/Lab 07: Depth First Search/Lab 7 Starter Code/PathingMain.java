...
public class PathingMain extends PApplet {
   ...
   private boolean depthFirstSearch(Point pos, GridValues[][] grid, List<Point> path) {
      ...
      if ((!withinBounds(pos, grid)) || (grid[pos.y][pos.x] == GridValues.OBSTACLE) || (grid[pos.y][pos.x] == GridValues.SEARCHED)){
         ...
         found = (depthFirstSearch(new Point(pos.x - 1, pos.y), grid, path)) || //check left
                 (depthFirstSearch(new Point(pos.x, pos.y + 1), grid, path)) || //check down
                 (depthFirstSearch(new Point(pos.x + 1, pos.y), grid, path)) || //check right
                 (depthFirstSearch(new Point(pos.x, pos.y - 1), grid, path));   //check up
      }
      if (found){
         path.add(0, pos);
      }
