...
/* 
    ...
    public static void processImageLine(Map<String, List<PImage>> images, String line, PApplet screen)
    {
        ...
        if (attrs.length >= 2) {
            ...
            if (img != null && img.width != -1) {
                ...
                if (attrs.length >= ImageStore.KEYED_IMAGE_MIN) {
            ...
            String[] properties, WorldModel world, ImageStore imageStore)
    {
        if (properties.length == Entity.SAPLING_NUM_PROPERTIES) {
            ...
            Entity entity = new Entity(EntityKind.SAPLING, id, pt, getImageList(imageStore, Entity.SAPLING_KEY), 0, 0,
            Entity.SAPLING_ACTION_ANIMATION_PERIOD, Entity.SAPLING_ACTION_ANIMATION_PERIOD, health, Entity.SAPLING_HEALTH_LIMIT);
            ...
            String[] properties, WorldModel world, ImageStore imageStore)
    {
        if (properties.length == Entity.DUDE_NUM_PROPERTIES) {
            Point pt = new Point(Integer.parseInt(properties[Entity.DUDE_COL]),
                    Integer.parseInt(properties[Entity.DUDE_ROW]));
            Entity entity = Entity.createDudeNotFull(properties[Entity.DUDE_ID],
                    ...
                    getImageList(imageStore, Entity.DUDE_KEY));
                    world.tryAddEntity(entity);
        }
            ...
            String[] properties, WorldModel world, ImageStore imageStore)
    {
        if (properties.length == Entity.FAIRY_NUM_PROPERTIES) {
            Point pt = new Point(Integer.parseInt(properties[Entity.FAIRY_COL]),
                    Integer.parseInt(properties[Entity.FAIRY_ROW]));
            Entity entity = Entity.createFairy(properties[Entity.FAIRY_ID],pt,
                                                ...
                                                getImageList(imageStore, Entity.FAIRY_KEY));
                    world.tryAddEntity(entity);
        }
            ...
            String[] properties, WorldModel world, ImageStore imageStore)
    {
        if (properties.length == Entity.TREE_NUM_PROPERTIES) {
            Point pt = new Point(Integer.parseInt(properties[Entity.TREE_COL]),
                    Integer.parseInt(properties[Entity.TREE_ROW]));
            Entity entity = Entity.createTree(properties[Entity.TREE_ID],
                    ...
                    getImageList(imageStore, Entity.TREE_KEY));
                    world.tryAddEntity(entity);
        }
            ...
            String[] properties, WorldModel world, ImageStore imageStore)
    {
        if (properties.length == Entity.OBSTACLE_NUM_PROPERTIES) {
            ...
            Entity entity = Entity.createObstacle(properties[Entity.OBSTACLE_ID], pt,
            ...
            String[] properties, WorldModel world, ImageStore imageStore)
    {
        if (properties.length == Entity.HOUSE_NUM_PROPERTIES) {
            Point pt = new Point(Integer.parseInt(properties[Entity.HOUSE_COL]),
                    Integer.parseInt(properties[Entity.HOUSE_ROW]));
            Entity entity = Entity.createHouse(properties[Entity.HOUSE_ID], pt,
                    getImageList(imageStore,
                    Entity.HOUSE_KEY));
            world.tryAddEntity(entity);
        }
            ...
            EventScheduler scheduler)
    {
        ...
        if (!entity.transformPlant(world, scheduler, imageStore)) {
            scheduleEvent(scheduler, entity,Action.createActivityAction(entity, world, imageStore),entity.getActionPeriod());
        }
    }
        ...
        if (fairyTarget.isPresent()) {
            ...
            if (entity.moveToFairy(world, fairyTarget.get(), scheduler)) {
                Entity sapling = Entity.createSapling("sapling_" + entity.id, tgtPos,
                        getImageList(imageStore, Entity.SAPLING_KEY));
            ...
            EventScheduler scheduler)
    {
                ...
                fullTarget.get(), scheduler))
        {
            ...
            scheduleEvent(scheduler, entity,Action.createActivityAction(entity, world, imageStore),entity.actionPeriod);
        }
    }
...
public static void executeAnimationAction(
            ...
            List<PImage> images)
    {
        ...
        Entity.SAPLING_ACTION_ANIMATION_PERIOD, Entity.SAPLING_ACTION_ANIMATION_PERIOD, 0, Entity.SAPLING_HEALTH_LIMIT);
    }
            ...
            Entity entity, WorldModel world, Point destPos)
    {
        ...
        if (horiz == 0 || world.isOccupied(newPos) && getOccupancyCell(world, newPos).kind != EntityKind.STUMP) {
            int vert = Integer.signum(destPos.y - entity.position.y);
            newPos = new Point(entity.position.x, entity.position.y + vert);

            if (vert == 0 || world.isOccupied(newPos) &&  getOccupancyCell(world, newPos).kind != EntityKind.STUMP) {
...
public static boolean transformTree(
            ...
            ImageStore imageStore)
    {
        if (entity.health <= 0) {
            Entity stump = createStump(entity.id,
                    entity.position,
                    getImageList(imageStore, Entity.STUMP_KEY));
